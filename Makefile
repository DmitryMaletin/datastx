...
antlr-build-looker:
	echo "Building $(ANTLR_IMAGE) docker image"
	echo "WORKDIR: $(WORKDIR)"
	docker build -t antlr4 $(WORKDIR) -f $(DOCKER_DIR)/Dockerfile.lookerantlr
...
antlr-jinja2: antlr-build-looker
	docker run -it -v $(PWD):/datastx antlr4 -Dlanguage=Go $(JINJA_MOUNT_PARSER_DIR)/Jinja.g4 $(PARSER_PATTERN) -o $(JINJA_MOUNT_PARSER_DIR)
